...
class Toric_code():
    ...
    def generate_random_error(self, p_error):
        '''
        Params : 
            p_error : float, ratio of generating errors
        '''
        for i in range(2):
    ...
    def generate_n_random_errors(self, n):
        '''
        Params:
            n : int, number of total errors (X + Y + Z)
        '''
    ...
    def syndrom(self, state):
        '''
        Update syndrom based on the self.qubit_matrix status for either current or next state
        Params :
            state : str, 'state' or 'next_state'. Update syndrom for either this or next state
        '''
        # generate vertex excitations (charge)
        # can be generated by z and y errors 
        qubit0 = self.qubit_matrix[0,:,:]        
        y_errors = (qubit0 == 2).astype(int) # separate y errors from x 
        z_errors = (qubit0 == 3).astype(int) # separate z errors from x 
